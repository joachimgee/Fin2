...
def compute_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    TODO(Phase 2): implement. Reference shape (every line shifted):
        out["log_ret_1"] = np.log(df["close"]).diff().shift(1)   # lookahead-safe
        out["sma_ratio"] = (df["close"] / df["close"].rolling(20).mean()).shift(1)
        out["rsi_14"]    = compute_rsi(df["close"], 14).shift(1)
        out["vol_20"]    = np.log(df["close"]).diff().rolling(20).std().shift(1)
    (each real implementation line carries its own # lookahead-safe: comment)
